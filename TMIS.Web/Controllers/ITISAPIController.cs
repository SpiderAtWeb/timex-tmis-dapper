...
namespace TMIS.Controllers
{
  ...
  [ApiController]
  public class ITISAPIController(IApproveRepository approveRepository, ILogger<ITISAPIController> logger) : ControllerBase
  {
    private readonly IApproveRepository _approveRepository = approveRepository;
    private readonly ILogger<ITISAPIController> _logger = logger;
    ...
    public async Task<ActionResult> Approve([FromQuery] string assignmentID, [FromQuery] string action)
    {
      ...
      try
      {
        ...
        if (assignmentInfo.AssignStatusID == 3 || assignmentInfo.AssignStatusID == 4)
        {
          ...
          var html = GenerateAlreadyProcessedHtml(iD, Devicestatus);
        ...
        if (result)
        {
          ...
          var successHtml = GenerateSuccessHtml(id.ToString(), mStatus);
    ...
    private static string GenerateAlreadyProcessedHtml(string assignmentID, string currentStatus)
    {
      return $@"
        <!DOCTYPE html>
        <html lang='en'>
    ...
    private static string GenerateSuccessHtml(string assignmentID, string status)
    {
      return $@"
        <!DOCTYPE html>
        <html lang='en'>
